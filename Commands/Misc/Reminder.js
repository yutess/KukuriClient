...
module.exports = {
    ...
    execute: async (message, args, client) => {
        ...
        if (args.length < 3) {
            return message.channel.send('Usage: reminder <dd/mm/yy> <HH:MM> <message>\nExample: reminder 01/01/22 12:00 Hey, remember to do something');
        }
        ...
        if (!/^\d{2}\/\d{2}\/\d{2}$/.test(dateStr)) {
            ...
            return message.channel.send('Invalid date format. Please use dd/mm/yy\nExample: reminder 01/01/22 12:00 Coding tonight');
        }    

        // Validate time format
        if (!/^\d{2}:\d{2}$/.test(timeStr)) {
            console.log(`Received dateStr: ${dateStr}`); // Debug
            return message.channel.send('Invalid time format. Please use HH:MM (24-hour format)\nExample: reminder 01/01/22 12:00 Playing game tonight');
        }
        ...
        try {
            ...
            if (isNaN(reminderDate.getTime())) {
                return message.channel.send('Invalid date/time provided.');
            }

            if (reminderDate <= new Date()) {
                return message.channel.send('Reminder time must be in the future.');
            }
            ...
            await message.channel.send(`Reminder set for ${dateStr} ${timeStr}: "${reminderMsg}"`);

        } catch (error) {
            console.error('Error setting reminder:', error);
            message.channel.send('An error occurred while setting the reminder.');
        }
    },
};